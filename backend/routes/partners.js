...
router.get('/partners', async (req, res) => {
    try {
        const partners = await Partner.find({ status: 'active' })
            .select('companyName website email contactPerson description slug') // PRIDĖTA: slug
            .sort({ createdAt: -1 });
...
router.get('/partners/pending', async (req, res) => {
    try {
        ...
        const partners = await PendingPartner.find(query)
            .select('companyName website email contactPerson description requestDate slug') // PRIDĖTA: slug
            .sort({ requestDate: -1 });
...
router.put('/partners/:id/approve', async (req, res) => {
    try {
        ...
        const newPartner = new Partner({
            ...
            slug: pendingPartner.slug, // PRIDĖTA: slug perduodamas
            status: 'active'
        });
...
router.get('/admin/pending-partners', async (req, res) => {
    try {
        ...
        const partners = await PendingPartner.find(query)
            .select('companyName website email contactPerson description requestDate ipAddress attempts slug') // PRIDĖTA: slug
            .sort({ requestDate: -1 })
            .limit(limit * 1)
            .skip((page - 1) * limit);
